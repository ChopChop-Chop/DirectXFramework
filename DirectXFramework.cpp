//-----------------------------------------------------------------------------
// File: DirectXFramework.cpp
//
// Desc: For advanced geometry, most apps will prefer to load pre-authored
//       DirectXFramework from a file. Fortunately, when using DirectXFramework, D3DX does most of
...
HRESULT InitGeometry()
{
                                   ...
                                   &g_pMesh ) ) )
    {
        ...
        if( FAILED( D3DXLoadMeshFromX( "..\\Tiger.x", D3DXMESH_SYSTEMMEM,
                                       ...
                                       &g_pMesh ) ) )
        {
            MessageBox( NULL, "Could not find tiger.x", "DirectXFramework.exe", MB_OK );
            return E_FAIL;
        }
    }
            ...
            lstrlenA( d3dxMaterials[i].pTextureFilename ) > 0 )
        {
                                                    ...
                                                    &g_pMeshTextures[i] ) ) )
            {
                ...
                if( FAILED( D3DXCreateTextureFromFileA( g_pd3dDevice,
                                                        ...
                                                        &g_pMeshTextures[i] ) ) )
                {
                    MessageBox( NULL, "Could not find texture map", "DirectXFramework.exe", MB_OK );
                }
            }
        }
    }
...
VOID Render()
{
    ...
    if( SUCCEEDED( g_pd3dDevice->BeginScene() ) )
    {
        ...
        // DirectXFramework are divided into subsets, one for each material. Render them in
        // a loop
        for( DWORD i = 0; i < g_dwNumMaterials; i++ )
        {
...
INT WINAPI wWinMain( HINSTANCE hInst, HINSTANCE, LPWSTR, INT )
{
    ...
    HWND hWnd = CreateWindow( "D3D Tutorial", "D3D Tutorial 06: DirectXFramework",
